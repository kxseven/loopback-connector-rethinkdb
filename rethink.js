...
RethinkDB.prototype._observe = function (model, filter, options, callback) {
  ...
  if (filter.limit) {
    filter.skip = Number(filter.skip) || 0;
    //https://github.com/rethinkdb/rethinkdb/issues/4909#issuecomment-227316397
    promise = promise.limit(filter.limit + filter.skip);
  }
  ...
  var observable = Rx.Observable.create(function (observer) {
    ...
    if (filter.skip) {
      //https://github.com/rethinkdb/rethinkdb/issues/4909#issuecomment-227316397
      promise = promise.filter(function (c) {
        return c.hasFields("old_offset").and(c('old_offset').gt(filter.skip - 1))
    ...
    try {
      promise.run(client).then(function (res) {
            ...
            feed.eachAsync(function (item) {
              if (!item.state) {
                item.old_offset = item.old_offset - filter.skip;
                item.new_offset = item.new_offset - filter.skip;
