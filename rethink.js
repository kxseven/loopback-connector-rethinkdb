...
RethinkDB.prototype.autoupdate = function (models, cb) {
  ...
  if (_this.db) {
    ...
    async.each(models, function (model, modelCallback) {
      ...
      r.db(_this.database).table(_this.tableName(model)).indexList().run(_this.db, function (error, alreadyPresentIndexes) {
...
RethinkDB.prototype.automigrate = _.debounce(function (models, done) {
  ...
  r.db(_this.database).tableList().run(client, function (error, list) {
    if (error) {
      return done(error);
    }
    var migratedTables = [];
...
RethinkDB.prototype.changeFeed = function (model, filter, options) {
  ...
  var observable = Rx.Observable.create(function (observer) {
    ...
    promise.run(client).then(function (res) {
      feed = res;
      var isReady = false;
      return feed.eachAsync(function (item) {
        if (item.state === 'ready') {
          isReady = true;
          sendResults();
...
RethinkDB.prototype.all = function all(model, filter, options, callback) {
  ...
  promise.run(client, function (err, data) {
    if (err) {
      return callback(err);
    }
